...
function _fit(
  ...
  # box constraints
  δ = 1e-8
  rₗ, rᵤ = isnothing(range′) ? (zero(rmax) + δ, rmax) : (range′ - δ, range′ + δ)
  sₗ, sᵤ = isnothing(sill′) ? (zero(smax) + δ, smax) : (sill′ - δ, sill′ + δ)
  nₗ, nᵤ = isnothing(nugget′) ? (zero(nmax) + δ, nmax) : (nugget′ - δ, nugget′ + δ)
  l = [rₗ, sₗ, nₗ]
  u = [rᵤ, sᵤ, nᵤ]
...
function _fit(
  ...
  # box constraints
  δ = 1e-8
  sₗ, sᵤ = isnothing(scaling′) ? (zero(smax) + δ, smax) : (scaling′ - δ, scaling′ + δ)
  nₗ, nᵤ = isnothing(nugget′) ? (zero(nmax) + δ, nmax) : (nugget′ - δ, nugget′ + δ)
  eₗ, eᵤ = isnothing(exponent′) ? (zero(emax) + δ, emax) : (exponent′ - δ, exponent′ + δ)
  l = [sₗ, nₗ, eₗ]
  u = [sᵤ, nᵤ, eᵤ]
